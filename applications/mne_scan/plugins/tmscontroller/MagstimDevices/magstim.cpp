...
MagStim::MagStim(QString portName, QObject *parent) : QObject(parent)
{
    this->m_robot = new ConnectionRobot(this->m_sendQueue, this->m_robotQueue);
    this->setupSerialPort(portName);
    this->m_connected = false;
    this->m_connectionCommand = std::make_tuple(QString("Q@n").toUtf8(), "", 3);

    //sender/receiver are pointers to QObjects and signal/slot are function signatures without parameter names.
    // QObject::connect(sender, signal, receiver, slot);
    // QObject::connect(const QObject *sender, const char *signal,
    //                  const QObject *receiver, const char *slot/member, Qt::ConnectionType::AutoConnection);

    // ConnectionRobot sends Message to SerialPortController for MagStim
    QObject::connect(this->m_robot, &ConnectionRobot::updateWriteQueue,
                     this->m_serial, &SerialPortController::updateWriteQueue, Qt::ConnectionType::DirectConnection);
...
void MagStim::arm(bool delay, messageInfo &message, int &error)
{
    //error = 0;
...
void MagStim::connect(int &error)
{
    ...
    if (!this->m_connected)
    {
        this->m_serial->start(QThread::Priority::TimeCriticalPriority);
        messageInfo mes;
        ...
        else
        {
            ...
            if (this->m_serial->isRunning())
            {
                this->m_serial->wait();
            }
            //throw "Could not establish remote control over the MagStim.";
            //std::cout << "Could not establish remote control over the MagStim. Error = " << error << std::endl;
            //abort(); //terminate(); exit(error);
            return;
        }
    } // if (!this->m_connected)
}
...
void MagStim::disarm(messageInfo &message, int &error)
{
    error = this->processCommand("EA", "instr", 3, message);
    return;
}
...
void MagStim::fire(messageInfo &message, int &error)
{
    //error = 0;
    //QString str = "instr";
    error = this->processCommand("EH", "instr", 3, message);
    return;
}

//============================================================================================================= getParameters()

messageInfo MagStim::getParameters(int &error)
{
    // error = 0;
    messageInfo mes;
...
messageInfo MagStim::getTemperature(int &error)
{
    // error = 0;
    messageInfo mes;
...
//============================================================================================================= parseMagstimResponse()

messageInfo MagStim::parseMagstimResponse(std::list<int> responseString, QString responseType)
{
    messageInfo magstimResponse;
    ...
    {
        // Get ASCII code for first data character
    ...
    else if (responseType == "rapidParam")
    {
        ...
        {
            // FW: TODO not quite sure, if this is correct for software version < 7 as well
...
std::tuple<int, int, int> MagStim::parseMagstimResponse_version(std::list<int> responseString)
{
    ...
    // File with zeros to get subsub version
...
void MagStim::setupSerialPort(QString portName)
{
    this->m_serial = new SerialPortController(portName, this->m_sendQueue, this->m_receiveQueue);
    // Read response from MagStim
    QObject::connect(this->m_serial, &SerialPortController::updateReadQueue,
                     this, &MagStim::updateReceiveQueue);
    // Write message to MagStim
    QObject::connect(this, &MagStim::updateSendQueue,
                     this->m_serial, &SerialPortController::updateWriteQueue);
    // connect(sender, signal, receiver, slot), the order cannot be adjusted.
}

//============================================================================================================= processCommand()

int MagStim::processCommand(QString commandString, QString receiptType, int readBytes,
                            messageInfo &message)
{
...
int MagStim::processCommand(QString commandString, QString receiptType, int readBytes,
                            std::tuple<int, int, int> &version, messageInfo &message)
{
    QByteArray comString = commandString.toLocal8Bit();
    QByteArray reply;

    if (this->m_connected || comString.at(0) == char(81) || comString.at(0) == char(82) ||
            commandString.at(0) == char(74) || comString.at(0) == char(70) ||
            commandString.contains("EA") || (comString.at(0) == char(92) && this->m_parameterReturnByte != 0))
    {
        sendInfo info;
        ...
        {
            m_eventLoop.exec();
            int error = std::get<0>(this->m_receiveQueue.head());
            reply = std::get<1>(this->m_receiveQueue.head());
            this->m_receiveQueue.dequeue();
            if (error)
            {
                return error;
            }
            else
            {
                ...
                else if (reply.at(0) != commandString.at(0))
                {
        ...
        if (this->m_connected)
        {
            if (commandString.at(0) == char(82))
            {
                emit updateRobotQueue(-1);
            }
            else if (commandString.left(2).contains("EA"))
            {
                emit updateRobotQueue(1);
            }
            else if (commandString.left(2).contains("EB"))
            {
        ...
        return 0;
    } // if (this->m_connected || comString.at(0) == char(81) || ... this->m_parametreReturnByte != 0))
    else
    {
...
{
    if (silent & this->m_connected) // if (silent && this->m_connected) ???
    {
        emit updateRobotQueue(0);
    }
    else
    {
        int error = 0;
        messageInfo mes;    // std::map<QString, std::map<QString, double>> mes;
        remoteControl(true, mes, error);
    }
    return;
}
...
void MagStim::remoteControl(bool enable, messageInfo &message, int &error)
{
    //error = 0;
    //QString str = "instr";
    if (enable)
...
void MagStim::setPower(int newPower, bool delay, int &error, QString commandByte, messageInfo &message)
{
    // error = 0;
    ...
    if (delay && !error)
    {
        if (!error)
        {
            ...
            {
                QThread::msleep((priorPower - newPower) * 10);
            }
        }
        else
        {
...
void MagStim::updateReceiveQueue(receiveInfo info)
{
    this->m_receiveQueue.enqueue(info);
    emit readInfo();
}
